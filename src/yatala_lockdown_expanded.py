...
class UIRenderer:
    ...
    def draw_ascii_art(self, y: int, x: int, art_name: str, colour: int = 7) -> None:
        """Draw ASCII art at position"""
        if art_name not in ASCII_ARTS:
            return
        
        art = ASCII_ARTS[art_name]
        lines = art.strip().split('\n')
        
        attr = curses.color_pair(colour)
        
        for i, line in enumerate(lines):
            if y + i < self.height and x < self.width:
                try:
                    # Truncate line if it's too long
                    if x + len(line) > self.width:
                        line = line[:self.width - x - 1]
                    self.stdscr.addstr(y + i, x, line, attr)
                except curses.error:
                    pass  # Ignore drawing errors for out-of-bounds text
...
# Fingerprint: e3fcdea43598f95a9c51eba4686ab62c
# Copyright © 2025 NovaSysErr-X. All rights reserved.
